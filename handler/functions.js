...
async function swap_pages2(client, interaction, embeds) {
  ...
  let button_back = new MessageButton().setStyle('PRIMARY').setCustomId('1').setEmoji("◀️").setLabel("Back")

  let button_forward = new MessageButton().setStyle('PRIMARY').setCustomId('3').setEmoji('▶️').setLabel("Forward")
  ...
  collector.on("end", (reason) => {
    if(reason != "stopped"){
      swapmsg.edit({content: `:x: Time has ended, the buttons are now disabled!`, embeds: [embeds[currentPage]], components: getDisabledComponents(swapmsg.components)}).catch(() => {});
    }
  })
